...
require "onbnet.socket"

local socketchannel	= require "onbnet.socketchannel"
local onbnet = require "onbnet"
local driver = require "onbnet.mongodriver"
...
function auth_method:auth_scram_sha1(username,password)
	...
	if not string.sub(rnonce, 1, 12) == nonce then
		-- onbnet.error("Server returned an invalid nonce.")
		return false
	end
	...
	if parsed_t['v'] ~= server_sig then
		-- onbnet.error("Server returned an invalid signature.")
		return false
	end
	if not r.done then
		...
		if not r.done then
			-- onbnet.error("SASL conversation failed to complete.")
			return false
		end
	end
	return true
