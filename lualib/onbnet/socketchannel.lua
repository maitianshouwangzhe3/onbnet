...
local function check_connection(self)
	if self.__sock then
		...
		if socket.disconnected(self.__sock[1]) then
			-- closed by peer
			error("socket: disconnect detected ", self.__host, self.__port)
...
local function pop_response(self)
	while self.__sock do
		...
		onbnet.wait(self.__wait_response)
	end
end
...
local function autoclose_cb(self, fd)
	...
	if self.__wait_response and sock and sock[1] == fd then
		-- closed by peer
		error("socket closed by peer : ", self.__host, self.__port)
		close_channel_socket(self)
	end
end
...
local function connect_once(self)
	...
	local function _next_addr()
		...
		if addr then
			error("socket: connect to backup host", addr.host, addr.port)
		end
		return addr
	end
	...
	local function _connect_once(self, addr)
				...
				if id == self.__sock[1] then
					if size == 0 then
						...
						if not self.__overload then
							...
							error(string.format("WARNING: %d K bytes need to send out (fd = %d %s:%s)", size, id, self.__host, self.__port))
						end
					end
				end
			end

			onbnet.fork(overload_trigger, fd, 0)
		...
		self.__dispatch_thread = onbnet.fork(function()
			if self.__sock then
		...
		if self.__auth then
			...
			if not ok then
				...
				if message ~= socket_error then
					...
					error("socket: auth failed", message)
				end
			end
			self.__authcoroutine = false
			if ok then
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			if not once then
				error("socket: connect to", self.__host, self.__port)
			end
			return
		elseif once then
			return err
		...
		if t > 1000 then
			error("socket: try to reconnect", self.__host, self.__port)
			onbnet.sleep(t)
			t = 0
		else
			onbnet.sleep(t)
...
function channel:request(request, response, padding)
	...
	if padding then
		...
		local ret = socket_write(fd , request)
		if ret <= 0 then
			sock_err(self)
		end
	end
...
channel_socket.read = wrapper_socket_function(socket.recv)
channel_socket.readline = wrapper_socket_function(socket.recvline)
