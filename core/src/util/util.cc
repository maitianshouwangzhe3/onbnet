...
onbnet::util::cstr_explode::~cstr_explode() {
...
char* onbnet::util::replace_str(char *src, char old_char, char new_char) {
    if (NULL == src) {
        return NULL;
    }
...
std::string onbnet::util::int2string(uint32_t user_id) {
    std::stringstream ss;
    ss << user_id;
    return ss.str();
}

uint32_t onbnet::util::string2int(const std::string &value) {
    return (uint32_t)atoi(value.c_str());
}

// 由于被替换的内容可能包含?号，所以需要更新开始搜寻的位置信息来避免替换刚刚插入的?号
void onbnet::util::replace_mark(std::string &str, std::string &new_value, uint32_t &begin_pos) {
...
void onbnet::util::replace_mark(std::string &str, uint32_t new_value, uint32_t &begin_pos) {
    std::stringstream ss;
    ss << new_value;
...
inline unsigned char onbnet::util::to_hex(const unsigned char &x) {
    return x > 9 ? x - 10 + 'A' : x + '0';
}

inline unsigned char onbnet::util::from_hex(const unsigned char &x) {
    return isdigit(x) ? x - '0' : x - 'A' + 10;
}

std::string onbnet::util::URL_encode(const std::string &in) {
    std::string out;
...
std::string onbnet::util::URL_decode(const std::string &in) {
    std::string out;
    for (size_t ix = 0; ix < in.size(); ix++) {
        unsigned char ch = 0;
        if (in[ix] == '%') {
...
int64_t onbnet::util::get_file_size(const char *path) {
    int64_t filesize = -1;
...
const char* onbnet::util::mem_find(const char *src_str, size_t src_len, const char *sub_str,
